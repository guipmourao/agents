...
from .json_utils import canonicalize_iso_timestamp
...
def command_transaction_recover(args: argparse.Namespace) -> int:
    ...
    _emit(
        {
            "schema": "codex-brain.transaction-recovery.v1",
            "recovered": recovered,
...
def command_contracts(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        implicit_absence = (
            args.vault is None
            and not os.environ.get("CODEX_BRAIN")
...
def command_capture_plan(args: argparse.Namespace) -> int:
    ...
    _emit(
        {
            "schema": "codex-brain.capture-plan.v1",
            "status": "dry-run",
            "items": plans,
            "summary": {
                "items": len(plans),
...
def command_capture_apply(args: argparse.Namespace) -> int:
    ...
    results = [
        {
            "schema": "codex-brain.filesystem-capture.v1",
    ...
    _emit(
        {
            "schema": "codex-brain.capture-result.v1",
            "status": "applied",
            "items": results,
...
def command_capture_queue_list(args: argparse.Namespace) -> int:
    _emit(
        {
            "schema": "codex-brain.capture-queue-list.v1",
...
def command_capture_queue_resume(args: argparse.Namespace) -> int:
    ...
    _emit({"schema": "codex-brain.capture-queue-resume.v1", "entries": entries})
    return 0
...
def command_mode_set(args: argparse.Namespace) -> int:
    ...
    if not args.apply:
        _emit(
            {
                "schema": "codex-brain.mode-plan.v1",
                "status": "dry-run",
                "mode": args.mode,
                "plan": plan,
                "operation": operation,
...
def command_extension_dragonscale(args: argparse.Namespace) -> int:
    ...
    if args.check:
        ...
        _emit(
            {
                "schema": "codex-brain.extension-check.v1",
    ...
    if not operation["writes"]:
        _emit(
            {
                "schema": "codex-brain.extension-plan.v1",
                "extension": "dragonscale",
                "status": "noop",
                "changed_paths": [],
            }
...
def command_migrate(args: argparse.Namespace) -> int:
    ...
    if not operation["writes"]:
        _emit(
            {
                "schema": "codex-brain.migration-plan.v1",
                "status": "noop",
                "vault_layout": "v1",
                "changed_paths": [],
            }
...
def _timestamp_argument(args: argparse.Namespace) -> str:
    if args.generated_at:
        return canonicalize_iso_timestamp(args.generated_at)
...
def command_init(args: argparse.Namespace) -> int:
    ...
    if not operation["writes"]:
        _emit(
            {
                "schema": "codex-brain.initialization-plan.v1",
                "status": "noop",
                "scan": scan,
                "changed_paths": [],
            }
...
def command_adopt(args: argparse.Namespace) -> int:
    ...
    if not operation["writes"]:
        _emit(
            {
                "schema": "codex-brain.adoption-plan.v1",
                "status": "noop",
                "scan": scan_vault(destination),
                "changed_paths": [],
            }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="codex-brain")
