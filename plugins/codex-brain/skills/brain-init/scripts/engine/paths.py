    ...
    *,
    ...
    if not allow_uninitialized and not is_adoptable_vault(resolved):
        raise VaultSelectionError(
            ...
            f"{resolved} is not a codex-brain or Obsidian vault",
        )
    return resolved
...
def resolve_vault_root(
    ...
    *,
    ...
    Precedence: explicit argument, ``CODEX_BRAIN``, nearest workspace
    ...
    if explicit is not None:
        ...
        source = "explicit"
    elif env.get("CODEX_BRAIN"):
        candidate = canonical(env["CODEX_BRAIN"])
        source = "environment"
    else:
        ...
        if configured is not None:
            ...
            if nearest is None:
                raise VaultSelectionError(
                    "VAULT_NOT_FOUND",
                    "no vault selected; pass --vault or set CODEX_BRAIN",
                )
            candidate = nearest
