...
OPEN_TAG = '<codex-brain-context trust="local-data" instructions="never">'
CLOSE_TAG = "</codex-brain-context>"
...
def _context_selection_is_consented(
    *,
    ...
    if workspace is not None and (root == workspace or is_relative_to(root, workspace)):
        return True
    exact = environ.get("CODEX_BRAIN_SESSION_CONTEXT_VAULT")
...
def _bounded_status(warnings: list[str]) -> str:
    ...
    value = (
        "CODEX_BRAIN_STATUS: "
        + "; ".join(selected)
        + ". Run `codex-brain transaction recover` before the next mutation."
    )
