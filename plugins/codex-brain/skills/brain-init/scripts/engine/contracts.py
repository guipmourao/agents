...
from engine.json_utils import strict_json_loads
...
def _validate_verification_semantics(
    ...
    invokes_contract_module = any(
        "engine/contracts.py" in token for token in normalized
    )
    invokes_portable_cli = any(
        "scripts/codex-brain.py" in token for token in normalized
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate and inspect codex-brain contracts"
    )
