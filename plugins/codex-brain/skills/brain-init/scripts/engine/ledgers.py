...
SOURCE_SCHEMA = "codex-brain.source-ledger.v1"
CLAIM_SCHEMA = "codex-brain.claim-ledger.v1"
...
def validate_existing_canonical_state(
    ...
    *,
    ...
    workspace = _existing_json(root, ".codex-brain.json")
    if workspace is not None:
        if workspace.get("schema") != "codex-brain.workspace.v1":
            _error(
                errors, ".codex-brain.json.schema", "unsupported workspace schema"
            )
        configured = workspace.get("vault")
        if not isinstance(configured, str) or not configured.strip():
            _error(errors, ".codex-brain.json.vault", "must be a non-empty path")
        else:
...
def migration_bundle(
    ...
    *,
    ...
    values = {
        ...
        CLAIM_PATH: claims,
        ".codex-brain.json": {
            "schema": "codex-brain.workspace.v1",
            "vault": ".",
            "role": "vault",
            "source_inbox": "inbox",
            "legacy_raw": ".raw",
